# server.py - V3 with Web Search Support
...
def distort(capture: Capture):
    ...
    # V3: Enrich signal with web search if intent detected
    signal = enrich_signal_with_web(signal)
...
def chat_followup(req: ChatFollowUpRequest):
    ...
    # V3: Check if user message has web search intent
    signal = Signal(
        id=str(uuid.uuid4()),
        content=req.message,
        source="chat-followup",
        captured_at=datetime.now(timezone.utc).isoformat(),
        tags=[],
        metadata={"session_id": session.session_id}
    )
    enriched_signal = enrich_signal_with_web(signal)
    
    # Use enriched content for the message
    actual_message = enriched_signal.content
    
    # Temporarily add enriched message to session for prompt building
    session.add_message("user", actual_message)
    
    # Build prompt with conversation history (including enriched message)
    prompt = build_chat_prompt(session, actual_message)
