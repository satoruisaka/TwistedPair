...
def extract_search_query(text: str) -> Optional[str]:
    ...
    Strategies:
    1. Look for explicit "for/about X" after trigger phrase
    2. Remove trigger phrases and use remaining text as query
    3. Use full text if no clear extraction
    ...
    # Strategy 1: Look for "for X" or "about X" patterns
    ...
    # Strategy 2: Remove trigger phrases and use what's left
    cleaned = text
    for pattern in COMPILED_PATTERNS:
    ...
    # Strategy 3: Fallback to full text (remove just the trigger word)
    fallback = re.sub(r'\b(?:check|search|look|find|go)\b', '', text, flags=re.IGNORECASE)
    fallback = re.sub(r'\b(?:web|online|internet)\b', '', fallback, flags=re.IGNORECASE)
